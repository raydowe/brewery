...
class BeerDetailsParserTests: XCTestCase {
...
"""
        let parser = BeerDetailsParser()
        let beerDetailsJSON = parser.parse(beerDetailsJSON: input)
        XCTAssertEqual(beerDetailsJSON?.name, "Buzz")
        XCTAssertEqual(beerDetailsJSON?.abv, 4.5)
    }
}
